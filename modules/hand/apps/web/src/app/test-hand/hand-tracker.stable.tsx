...
export default function HandTrackingTestPage() {
  ...
  const drawFrame = (landmarks: Float32Array | null) => {
    ...
    if (sw.active || true) {
      ...
      // Red straight line: best fit through 4 centroids, aligned with sword axis
      const centsPos: {x:number;y:number}[] = [];
      for (const { joints, color } of fingerDef) {
        const [a,b,c,d] = joints;
        const cx = (landmarks[a*3]+landmarks[b*3]+landmarks[c*3]+landmarks[d*3]) / 4;
        const cy = (landmarks[a*3+1]+landmarks[b*3+1]+landmarks[c*3+1]+landmarks[d*3+1]) / 4;
      ...
      if (centsPos.length === 4) {
        ...
        const cosA = Math.cos(sw.angle), sinA = Math.sin(sw.angle);
        ctx.beginPath();
      ...
      ctx.lineWidth = 1;
      ctx.stroke();
      // Handle nodes = the 4 colored centroids (same positions as the colored dots)
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      setLogs([]);
      setSwordStatus("");
      swordRef.current = { active: false, swinging: false, swingStartTime: 0, trail: [], gripX: 0, gripY: 0, angle: 0, handleNodes: [], smoothS: 1, tiltX: 0, tiltY: 0 };
      ...
      // Initialize Three.js 3D scene
      {
        ...
        if (canvas && !threeRef.current) {
          ...
          scene.add(dl2);
          // Sword group (200-unit blade fits viewport; wider/thicker = visually bigger)
          ...
          sword.add(blade);
          // Marker points along the sword — 4 colored markers matching finger centroids
          ...
          sword.add(pommel);
          scene.add(sword);
            threeRef.current = { scene, camera: cam, renderer, sword, markerMeshes };
      ...
      const loop = async () => {
        ...
        try {
          ...
          if (frame?.landmarks) {
            ...
            // Best-fit line through the 4 finger centroids (PCA first principal component)
            let _xx = 0, _yy = 0, _xy = 0;
            for (const c of cents) { const dx = c.x - centroidCx, dy = c.y - centroidCy; _xx += dx * dx; _yy += dy * dy; _xy += dx * dy; }
            let _bfX = 1, _bfY = 0;
            if (Math.abs(_xx) + Math.abs(_yy) > 1e-10) {
              const _theta = Math.atan2(2 * _xy, _xx - _yy) / 2;
              _bfX = Math.cos(_theta); _bfY = Math.sin(_theta);
            }
            // Orient toward index finger (centroid 0) so sword points forward
            if (_bfX * (cents[0].x - centroidCx) + _bfY * (cents[0].y - centroidCy) < 0) { _bfX = -_bfX; _bfY = -_bfY; }
            const rawAngle = Math.atan2(_bfY, _bfX);
            ...
            if (sw.gripX === 0 && sw.gripY === 0) {
              ...
              sw.forwardX = Math.cos(rawAngle); sw.forwardY = Math.sin(rawAngle);
              sw.forwardZ = _uLen > 0.001 ? _uz/_uLen : 0;
            } else if (fistDetected) {
              // Position EMA smoothing
              const _posAlpha = 0.3;
              sw.gripX += _posAlpha * (centroidCx - sw.gripX);
              sw.gripY += _posAlpha * (centroidCy - sw.gripY);
              if (_spread >= 0.005) {
                // Angle EMA (freezes when spread < 0.005 — tight fist)
                const _alpha = Math.min(0.5, paramsRef.current.smoothBase * Math.min(1, (_spread - 0.005) / 0.01));
                let _diff = rawAngle - sw.angle;
                if (_diff > Math.PI) _diff -= Math.PI * 2;
                if (_diff < -Math.PI) _diff += Math.PI * 2;
                sw.angle += _alpha * _diff;
                // 3D forward: XY from sw.angle (aligns with red line always), Z from 3D finger depth
                const _fwdAlpha = 0.25;
                const _fpcaX = Math.cos(sw.angle), _fpcaY = Math.sin(sw.angle);
                ...
                // Palm normal EMA (perpendicular to palm, sword roll constraint)
                const _upAlpha = 0.2;
            ...
            // 4 handle nodes = the 4 colored centroids themselves (no projection)
            const cosA = Math.cos(sw.angle);
            const sinA = Math.sin(sw.angle);
            sw.handleNodes = cents.map((c, i) => ({
              x: c.x, y: c.y, idx: i,
              t: (c.x - sw.gripX) * cosA + (c.y - sw.gripY) * sinA,
              cx: c.x, cy: c.y,
            }));
            // Store debug data
            ...
            buf.push({
              ...
              conf: result.confidence, vel: velocity, tx: sw.tiltX, ty: sw.tiltY,
            ...
            if (canvas && container) {
              ...
              if (w > 0 && h > 0) {
                ...
                if (sw.active) {
                  ...
                  sword.scale.set(sw.smoothS, sw.smoothS, sw.smoothS);
